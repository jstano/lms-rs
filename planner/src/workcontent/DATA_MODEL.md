...
> the module needs to fully generate `WorkContent`/`LaborData` from business-driver-driven and salaried
...
- **`WorkContent`** — discrete blocks of required work on a date/time axis (e.g. "need 2 people,
  6:00–10:00, front desk"), computed from business-driver-driven volume forecasts distributed across the day.
  ...
  └─ for each Job (leaf Assignment in the labor-level tree)
       └─ resolve PlannerSettings (Assignment-level → Property-level → inherited/merged)
            └─ dispatch on StandardType
                 ...
                 ├─ ADVANCED  → business-driver-driven flow/spread/recurring-task distribution engine
...
|---|---|---|
| `NONE` | `NONE` | No-op. |
| `BASIC` | `SIMPLE_NON_FLOWED` | Builds `WorkContent` directly from standards using a fixed `NonFlowedDistributionMethod` (`BEGINNING`/`MIDDLE`/`END`/`EVEN`/`VARYING`) — no business driver flow curve needed. |
| `ADVANCED` | `KBI_RELATED` | Full engine: resolves business driver volume per date/environment, looks up the matching standard range/value, distributes it across time periods via a `DistributionSchedule`/`Distributor` keyed by `DistributionMethod`, applies breaks, collapses into `WorkContent` blocks per `PlanType`. |
...
units, standard sets, shift categories, revenue centers, business drivers, flow plans, and property-level
...
| `business_driver_set_id` | `Option<BusinessDriverSetId>` | |
...
| `PropertyDataKey` | `PropertyDataKey.java` | typed property config keys with defaults: `PlannerEngineMaxDurationForDynamicWork(4)`, `OverrideCurrentSystemDate`, `FlowAssumptionThreshold(1)`, `SyncArrivalTime(false)`, `SystemPlannedShiftAudit(false)`, `LegacyDistributionPatternRounding(false)`, `EnableDynamicStandardSets(false)` |
...
#### `OperatingCalendar` / `OperatingCalendarDate`
*Java: `CalendarPlan.java`, `CalendarPlanDate.java`* — **Reconciliation:** renamed from
`CalendarPlan`/`CalendarPlanDate` — this is definitional open/closed-day data, not a "plan".
Property-level open/closed calendar.

| `OperatingCalendar` field | Type | Description |
|---|---|---|
| `id` | `OperatingCalendarId` | |
| `property_id` | `PropertyId` | |
| `name` | `String` | |
| `year` | `u32` | |
| `open_days` | `[bool; 7]` | Sun..Sat open flags |
| `closed_dates` | `Vec<OperatingCalendarDateId>` | |

| `OperatingCalendarDate` field | Type | Description |
|---|---|---|
| `id` | `OperatingCalendarDateId` | |
| `operating_calendar_id` | `OperatingCalendarId` | |
...
#### `EnvStat`
*Java: `EnvStat.java`* — links a business driver/date to its forecast vs. actual `Environment`.
...
### 2.4 Business Driver / volume drivers

`BusinessDriver` (Java: `KBI`, Key Business Indicator) is the abstraction for "the volume that
drives labor" — covers, transactions, occupied rooms, etc. It is referenced pervasively by the
standards catalog (§2.7). **Reconciliation:** this section renames and extends Rust's existing
`BusinessDriver`/`BusinessDriverId` stub (`domain/business_driver.rs`, currently just
`{ id: BusinessDriverId, location_id: LocationId }`) rather than introducing a parallel type;
`BusinessDriverId` already exists and needs no change. The unused `BusinessDriverValues` stub
(`domain/business_driver_values.rs`, see §5) is superseded by `BusinessDriverStat`/
`DynamicSpreadValue` below, which model per-date values properly.

#### `BusinessDriver`
*Java: `KBI.java` (record)* — **Reconciliation:** extends the existing Rust `BusinessDriver`
struct; `location_id` is renamed `property_id` (see `Property`, §2.1) and `name`/`code`/`unit_id`
are added.

| Field | Type | Description |
|---|---|---|
| `id` | `BusinessDriverId` | unchanged |
| `property_id` | `PropertyId` | renamed from `location_id` |
| `name` / `code` | `String` | new |
| `unit_id` | `UnitId` | new |

#### `BusinessDriverSet`, `BusinessDriverConfig`, `BusinessDriverConfigEnvDistributionSchedule`, `BusinessDriverLaborConfig`
*Java: `KBISet.java`, `KBIConfig.java`, `KBIConfigEnvFlowPlan.java`, `KBILaborConfig.java`*

| `BusinessDriverSet` field | Type |
|---|---|
| `id` | `BusinessDriverSetId` |
| `property_id` | `PropertyId` |
| `name` | `String` |

| `BusinessDriverConfig` field | Type | Description |
|---|---|---|
| `id` | `BusinessDriverConfigId` | |
| `business_driver_id` | `BusinessDriverId` | |
| `business_driver_set_id` | `BusinessDriverSetId` | |
| `main_type` | `BusinessDriverType` | |
| `is_env` / `is_primary` | `bool` | |
| `notes` | `Option<String>` | |

| `BusinessDriverConfigEnvDistributionSchedule` field | Type |
|---|---|
| `id` | `BusinessDriverConfigEnvDistributionScheduleId` |
| `business_driver_config_id` | `BusinessDriverConfigId` |
| `environment_id` | `EnvironmentId` |
| `distribution_schedule_id` | `DistributionScheduleId` |

| `BusinessDriverLaborConfig` field | Type |
|---|---|
| `id` | `BusinessDriverLaborConfigId` |
| `business_driver_id` | `BusinessDriverId` |
| `distribution_schedule_id` | `DistributionScheduleId` |
| `actual_distribution_schedule_id` | `DistributionScheduleId` |

`BusinessDriverType` enum: `Calculated \| Separate \| Input \| PercentOfBase \| Statistical`.

#### `BusinessDriverStat`
*Java: `KBIStat.java`* — daily business driver value + forecast-accuracy record.

| Field | Type |
|---|---|
| `id` | `BusinessDriverStatId` |
| `business_driver_id` | `BusinessDriverId` |
| `property_id` | `PropertyId` |
| `date` | `LocalDate` |
...
#### `DynamicSpreadValue`, `TempDistributionPattern`
*Java: `DynamicSpreadValue.java`, `TempFlowPattern.java`* — scratch/computed values tied to a
business driver + date, used mid-calculation.
...
| `TempDistributionPattern` field | Type |
|---|---|
| `id` | `TempDistributionPatternId` |
| `business_driver_id` | `BusinessDriverId` |
| `date` | `LocalDate` |
| `distribution_pattern_id` | `DistributionPatternId` |
...
mechanism behind the `ADVANCED`/business-driver-related generator.

#### `DistributionPattern` / `DistributionPatternPeriod` / `DistributionPatternActual`
*Java: `FlowPattern.java`, `FlowPatternPeriod.java`, `FlowPatternActual.java`* —
**Reconciliation:** renamed from `FlowPattern`/`FlowPatternPeriod`/`FlowPatternActual`.

| `DistributionPattern` field | Type |
|---|---|
...
| `total_percent` | `f64` |
| `total_units` | `u32` |
| `periods` | `Vec<DistributionPatternPeriodId>` |

| `DistributionPatternPeriod` field | Type |
|---|---|
| `id` | `DistributionPatternPeriodId` |
| `distribution_pattern_id` | `DistributionPatternId` |
| `pattern_value` | `f64` |
| `actual_value` | `Option<i32>` |
| `period_no` | `u32` |

| `DistributionPatternActual` field | Type | Description |
|---|---|---|
| `id` | `DistributionPatternActualId` | |
| `distribution_pattern_id` | `DistributionPatternId` | |
| `stat_date` | `LocalDate` | |
| `period_no` | `u32` | |
| `actual_value` | `Option<i32>` | distinct per-date observation, vs. `DistributionPatternPeriod`'s single built-in value |

#### `DistributionSchedule`
*Java: `FlowPlan.java`* — **Reconciliation:** renamed from `FlowPlan`. Maps each day-of-week to a
`DistributionPattern` over a date range.
...
| `name` / `code` | `String` |
| `start_date` / `end_date` | `LocalDate` |
| `distribution_pattern_by_day` | `HashMap<DayOfWeek, DistributionPatternId>` (replaces Java's 7 discrete `sunFlowPattern..satFlowPattern` fields) |
...
the primary business-driver-driven "how much work" standard used by the `ADVANCED` generator.
...
`DynamicSpreadStandard.java`, `FilterableSpreadStandard.java`* — alternate business-driver-driven standard
...
with either fixed hours or a business-driver-volume-tiered formula.
...
from the business-driver-flow standards above.
...
| `operating_calendar_id` | `Option<OperatingCalendarId>` |
| `capture_ratio_business_driver_id` | `Option<BusinessDriverId>` |
...
#### `WorkContentDetail`
*Java: `WorkContentDetail.java`* — per-business-driver breakdown of a `WorkContent`'s hours.
...
| `work_content_log_id` | `WorkContentLogId` | |
| `business_driver_id` | `Option<BusinessDriverId>` | |
| `business_driver_value` | `Option<i32>` | |
...
| `distribution_pattern_id` | `Option<DistributionPatternId>` | |
| `distribution_pattern_actual_data_used` | `Option<bool>` | |
| `distribution_pattern_id_used_for_actual_data` | `Option<i32>` | |
...
4. **`BASIC` / non-flowed path** — for each `(date, AssignmentShift)`, evaluate each standard
   ...
   (`Beginning \| Middle \| End \| Even \| Varying`) directly into `WorkContent` blocks — no business
   driver flow curve needed. This directly replaces the current placeholder `"BASIC:{date}:SHIFT:{id}"`
   string markers in `BasicStandardsProcessorImpl`.
5. **`ADVANCED` / business-driver-related path** — for each `(date, AssignmentShift, Environment)`:
   - Resolve the business driver volume for the date (via `BusinessDriverStat`/
     `DynamicSpreadValue`/environment-aware `EnvStat`).
   ...
   - Select a `Distributor` strategy keyed by `DistributionMethod` (flowed via `DistributionSchedule`/
     `DistributionPattern`, fill-gaps, non-flowed, opening, closing, share-with) to spread the value across
...
| Area | Status | Notes |
|---|---|---|
| `PlannerModel`, `Job`, `JobShift`, `SalariedStandard`, `StandardSet`, `Location` | Implemented (partial) | Present but narrower than this design; `Location`→`Property`, `Job`/`JobShift` need the `Assignment` tree and id-referencing fixes described in §2.1/§2.6. |
| `BusinessDriver`, `BusinessDriverId` | Implemented (partial) | Existing id-only stub (`domain/business_driver.rs`); §2.4 extends it with `name`/`code`/`unit_id` and renames `location_id`→`property_id`. None of `BusinessDriverSet`/`Config`/`Stat`/`Type` exist yet. |
...
| `BASIC` generator | Implemented, parity-checked | Produces real `WorkContent` and `PlannedShift` through `basic_standards_processor` / `basic_calculator` / `basic_work_content_creator`. The placeholder-string implementation this table used to describe is gone. The Java engine's own Spock tables have been ported and pass — see `PARITY_AUDIT.md` §1, including the two deliberate narrowings still open. |
| `ADVANCED` generator | Generates | `generate_work` runs the full eight-step flowed pipeline and returns work content and planned shifts. Groups 0–J of `ADVANCED_GENERATOR_PLAN.md` are complete, including whole-plan acceptance scenarios rebuilt from the Java integration tests. Two things it needs that this port does not model: the seasonal calendar, supplied through the `EnvironmentResolver` provider — **without one, standards cost nothing and the generator plans nothing** — and the existing schedule, supplied through `AdvancedWorkGenerator::with_existing_planned_shifts`. |
| `Environment`, `DistributionPattern`/`DistributionSchedule`, `SpreadStandard`, `RecurringTaskStandard`, `JobMinMaxCoverage` | Implemented (narrow) | Added for the flowed generator, scoped to what it reads. The wider config trees (`BusinessDriverSet`/`Config`/`Stat`, `Environments`/`EnvStat`, `DistributionPatternActual`) are still absent, and environment-for-date resolution is supplied through an injectable `EnvironmentResolver` rather than modelled. |
...
- ~~`Job::planner_settings()` returns `PlannerSettings::default()`~~ — **stale**; it returns
  `self.planner_settings` and is pinned by a test.
- ~~`JobShiftDefinition.start_time`/`end_time` are typed `LocalDate`~~ — **stale**; both are
  `Option<LocalTime>`.
- ~~`JobShiftDefinition` embeds a full `JobShift` by value~~ — **stale**; it holds no reference
  to its shift at all. The opposite problem, in fact: `GeneratorParameters` has to carry the
  `JobShift` and `JobShiftDefinition` separately, where Java reaches the shift through the
  detail.
- ~~`common/numbers.rs::round_raw_hours` rounded to 2 decimals~~ — **fixed**; Java's
  `TDouble.roundRawHours` is 4 decimals, confirmed against a Spock assertion
  (`roundRawHours(total) == 149.0008`). This was a live precision bug in
  `total_work_minutes_calculator.rs`.
...
- `BusinessDriverValues` (`domain/business_driver_values.rs`) is unused/superseded by
  `PlannerModel.business_driver_values: HashMap<BusinessDriverId, u32>`; in this design, per-date
  business driver values belong on `BusinessDriverStat`/`DynamicSpreadValue` (§2.4) instead.
...
| `CalendarPlan` | `OperatingCalendar` (renamed) | §2.3 |
| `CalendarPlanDate` | `OperatingCalendarDate` (renamed) | §2.3 |
...
| `FlowPattern` | `DistributionPattern` (renamed) | §2.5 |
| `FlowPatternActual` | `DistributionPatternActual` (renamed) | §2.5 |
| `FlowPatternPeriod` | `DistributionPatternPeriod` (renamed) | §2.5 |
| `FlowPlan` | `DistributionSchedule` (renamed) | §2.5 |
...
| `KBI` | `BusinessDriver` (renamed; extends existing Rust stub) | §2.4 |
| `KbiId` | `BusinessDriverId` (unchanged — already exists) | §2.4 |
| `KBIConfig` | `BusinessDriverConfig` | §2.4 |
| `KBIConfigEnvFlowPlan` | `BusinessDriverConfigEnvDistributionSchedule` (renamed) | §2.4 |
| `KBILaborConfig` | `BusinessDriverLaborConfig` | §2.4 |
| `KBISet` | `BusinessDriverSet` | §2.4 |
| `KBIStat` | `BusinessDriverStat` | §2.4 |
| `KBIType` | `BusinessDriverType` enum | §2.4 |
...
| `TempFlowPattern` | `TempDistributionPattern` (renamed) | §2.4 |
| `Unit` | `Unit` | §2.4 |
